...
static int settings_loop(int id, void *context, const input_data *input) {
  ...
  switch (id) {
    ...
    case SETTINGS_DISABLE_POWERSAVE:
      ...
      break;
    case SETTINGS_ENABLE_VSYNC:
      if ((input->buttons & SCE_CTRL_CROSS) == 0 || input->buttons & SCE_CTRL_HOLD) {
        break;
      }
      did_change = 1;
      config.enable_vsync = !config.enable_vsync;
      break;
...
#define MENU_REPLACE(ID, MESSAGE) \
  ...
  sprintf(current, "%s", config.enable_vsync ? "yes" : "no");
  MENU_REPLACE(SETTINGS_VIEW_ENABLE_VSYNC, current);
...
#define LEFT_RIGHT_ARROWS "\xe2\x86\x90\xe2\x86\x92"
  ...
  MENU_ENTRY(SETTINGS_ENABLE_VSYNC, SETTINGS_VIEW_ENABLE_VSYNC, "Enable V-Sync", "");
