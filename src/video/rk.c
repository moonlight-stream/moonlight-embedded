...
struct {
    int fd;
    uint32_t plane_id, crtc_id, cursor_plane_id;
    int frm_eos;
    int crtc_width;
    int crtc_height;
    RK_U32 frm_width;
...
void *display_thread(void *param) {
    ...
    int ret;
    fprintf(stdout, "DISPLAY THREAD START\n");
    ...
    while (!drm.frm_eos) {
        ...
        while (drm.fb_id == 0) {
            ...
            if (drm.fb_id == 0 && drm.frm_eos) {
                ...
                assert(!ret);
                fprintf(stdout, "DISPLAY THREAD END\n");
                return NULL;
            }
        }
        fb_id = drm.fb_id;
        if (drm.skipped_frames) {
            fprintf(stdout, "DISPLAY skipped %d\n", drm.skipped_frames);
        }
        drm.fb_id = 0;
...
void *frame_thread(void *param) {
    ...
    fprintf(stdout, "FRAME THREAD START\n");
    while (!drm.frm_eos) {
        ...
        fprintf(stdout, "GETFRAME %3d.%06d frid %d\n", ats.tv_sec, ats.tv_nsec / 1000, frid);
#endif
        if (frame) {
            if (mpp_frame_get_info_change(frame)) {
                ...
                if (buffer) {
                    ...
                    assert(i != MAX_FRAMES);
#ifdef _DEBUG
                    fprintf(stdout, "FRAME %d.%06d first %8jd previous %6jd frid %d\n", ats.tv_sec, ats.tv_nsec/1000,
...
int rk_setup(int videoFormat, int width, int height, int redrawRate, void* context, int drFlags) {
    ...
    // disable cursor
    drmModeSetCursor(drm.fd, drm.crtc_id, 0, 0, 0);

    // MPI SETUP

    pkt_buf = malloc(READ_BUF_SIZE);
    assert(pkt_buf);
    ret = mpp_packet_init(&mpi.packet, pkt_buf, READ_BUF_SIZE);
    assert(!ret);
...
int rk_submit_decode_unit(PDECODE_UNIT decodeUnit) {
    ...
    if (decodeUnit->fullLength < READ_BUF_SIZE) {
        ...
        if (length) {
            mpp_packet_set_pos(mpi.packet, pkt_buf);
            mpp_packet_set_length(mpi.packet, length);

            while (MPP_OK != mpi.mpi->decode_put_packet(mpi.ctx, mpi.packet)) {
                ;
            }
        }
    }
